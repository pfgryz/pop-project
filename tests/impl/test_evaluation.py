...
def test_wrs_for_empty_sleigh():
    assert weighted_reindeer_weariness(Matrix(), SimpleGiftManager([])) == 0


def test_wrs_for_single_gift_without_weight():
    gift_manager = SimpleGiftManager([(0, (69.9496, -17.0366))])
    assert weighted_reindeer_weariness(Matrix([0]), gift_manager) == \
...
def test_wrs_for_two_gifts():
    ...
    assert weighted_reindeer_weariness(Matrix([0, 1]), gift_manager) == approx(
        423080.2, abs=0.1)
